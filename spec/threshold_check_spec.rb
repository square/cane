...
describe Cane::ThresholdCheck do
  ...
  context "checking violations" do
    ...
    context 'when coverage threshold is valid' do
      ...
      context '>' do
        ...
        it { expect(run(:gt, 8)).to have_violation('x is 8.0, should be > 8.0') }
        it { expect(run(:gt, 9)).to have_violation('x is 8.0, should be > 9.0') }
      end

      context '>=' do
        it { expect(run(:gte, 7)).to have_no_violations }
        it { expect(run(:gte, 8)).to have_no_violations }
        it { expect(run(:gte, 9)).to have_violation('x is 8.0, should be >= 9.0') }
      end

      context '==' do
        it { expect(run(:eq, 7)).to have_violation('x is 8.0, should be == 7.0') }
        it { expect(run(:eq, 8)).to have_no_violations }
        it { expect(run(:eq, 9)).to have_violation('x is 8.0, should be == 9.0') }
      end

      context '<=' do
        it { expect(run(:lte, 7)).to have_violation('x is 8.0, should be <= 7.0') }
        it { expect(run(:lte, 8)).to have_no_violations }
        it { expect(run(:lte, 9)).to have_no_violations }
      end

      context '<' do
        it { expect(run(:lt, 7)).to have_violation('x is 8.0, should be < 7.0') }
        it { expect(run(:lt, 8)).to have_violation('x is 8.0, should be < 8.0') }
  ...
  context "normalizing a user supplied value to a threshold" do
    ...
    it 'normalizes a json file to a float' do
      expect(subject.normalized_limit(make_file(simplecov_last_run))).to eq(93.88)
    end
